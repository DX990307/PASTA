...
type PWqueue struct {
	...
	ResumeLevel         int
	ResumeNodePA        uint64
	CoalescingLocked    bool
	coalescingCandidate bool
}
...
type MMU struct {
	...
	neighborhoodCoalescingEnabled bool
...
func (mmu *MMU) doPageWalkHit(
	...
	madeProgress = mmu.coalescePendingRequests(now, walking) || madeProgress
...
func (mmu *MMU) processTranslationReqs(now sim.VTimeInSec) bool {
	...
	for i := 0; i < mmu.maxRequestsInFlight; i++ {
		...
		pwIndex := mmu.pickNextWalkRequestIndex()
		if pwIndex < 0 {
			break
		}

		pw := mmu.PWqueue[pwIndex]
		mmu.PWqueue = append(mmu.PWqueue[:pwIndex], mmu.PWqueue[pwIndex+1:]...)
