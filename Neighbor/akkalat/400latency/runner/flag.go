...
var gmmuPTELookupLatency = flag.Int("gmmu-pte-lookup-latency", 80,
	"Fixed GMMUCache PTE lookup/fill latency per returned PTE response, in cycles.")
var mmutlbLookupLatency = flag.Int("mmutlb-lookup-latency", 80,
	"Fixed MMUTLB/IOTLB lookup latency per requested PTE, in cycles, applied before each buffered translation request is looked up.")
var mmuNeighborhoodAwareCoalescing = flag.Bool("mmu-neighborhood-aware-coalescing", false,
	"Enable neighborhood-aware page-walk coalescing in the MMU backend.")
