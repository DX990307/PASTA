...
BASELINE_FLAGS = []
NEIGHBORHOOD_FLAGS = [
    "-mmu-neighborhood-aware-coalescing",
]
...
def parse_args():
    ...
    parser.add_argument(
        ...
        dest="only_config",
        default="",
        help="Only run experiments with this config name (e.g. 'baseline').",
    )
    parser.add_argument(
        "--rerun-missing",
        dest="rerun_missing",
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Comma-separated config list. Neighbor choices: "
            + ",".join(NEIGHBOR_CONFIG_NAMES)
            + ". Photon sampled choices: "
            + ",".join(name for name, _ in CONFIGS)
            + ". Use photon_all/all for grouped sampled configs."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Pass -max-wg to each benchmark. 0 disables the default cap for "
    ...
    parser.add_argument(
        ...
        help="Fixed MMUTLB/IOTLB lookup latency per requested PTE, in cycles, applied before each buffered translation request is looked up.",
    )
    parser.add_argument(
        ...
        help="Fixed GMMUCache PTE lookup/fill latency per returned PTE response, in cycles.",
    )
...
def build_ablation_configs(args):
    neighbor_configs = build_neighbor_config_map(args)
    if args.configs:
        return build_selected_configs(args, neighbor_configs)

    return [(name, neighbor_configs[name]) for name in NEIGHBOR_CONFIG_NAMES]


def build_neighbor_config_map(args):
    return {
        "baseline": BASELINE_FLAGS,
        "neighborhood": NEIGHBORHOOD_FLAGS,
    }


def build_selected_configs(args, neighbor_configs):
    requested = parse_csv(args.configs)
    photon_configs = {name: flags for name, flags in CONFIGS}
    selected = []

    explicit_neighbor = any(
        name in neighbor_configs and name != "baseline"
    ...
    for name in requested:
        ...
        if name == "baseline":
            if explicit_neighbor and not explicit_photon:
                selected.append(("baseline", neighbor_configs["baseline"]))
            else:
                selected += build_photon_configs(args, ["baseline"])
            continue

        if name in neighbor_configs:
            selected.append((name, neighbor_configs[name]))
            continue
...
def create_output_dir():
    ...
    output_dir = os.path.join(
        ROOT_DIR,
        "results",
        datetime.now().strftime("%Y-%m-%d-%H-%M-%S-neighbor-baseline"),
    )
