...
var (
	...
	rowsFlag = flag.Int(
		"rows", 1, "Row count, usually batch-size * seq-len.")
	colsFlag = flag.Int(
		"cols", 0, "Column count for row-softmax, or width for batchnorm2d.")
	hiddenFlag = flag.Int(
		"hidden", 0, "Transformer hidden size.")
	inputDimFlag = flag.Int(
		"input-dim", 0, "Input dimension for linear.")
	outputDimFlag = flag.Int(
		"output-dim", 0, "Output dimension for linear, or intermediate size for mlp.")
