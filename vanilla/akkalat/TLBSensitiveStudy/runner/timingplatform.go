...
func (b R9NanoPlatformBuilder) createMMU(
	...
	mmuBuilder := mmu.MakeBuilder().
		...
		WithMaxNumReqInFlight(*ptwCounts).
		WithPageTable(pageTable).
	...
	// fmt.Printf("MMU created with max %d PTWs in flight\n", *ptwCounts)
...
func (b *R9NanoPlatformBuilder) createIOMMUTLB(pageTable vm.PageTable) {
	numEntry := *IOTLBEntries
	numWays := 16
	numSets := numEntry / numWays
...
func (b *R9NanoPlatformBuilder) connIOMMUCacheWithIOMMUTLB() {
	...
	conn.PlugIn(b.IOMMUTLB.GetPortByName("Bottom"), 640)
	conn.PlugIn(b.IOMMUCache.GetPortByName("Top"), 640)
}
