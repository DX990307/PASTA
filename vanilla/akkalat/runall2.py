...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Comma-separated config list. Vanilla choices: "
            + ",".join(VANILLA_CONFIG_NAMES)
...
def build_ablation_configs(args):
    vanilla_configs = build_vanilla_config_map(args)
    if args.configs:
        return build_selected_configs(args, vanilla_configs)

    return [(name, vanilla_configs[name]) for name in VANILLA_CONFIG_NAMES]


def build_vanilla_config_map(args):
    return {
        "baseline": BASELINE_FLAGS,
    }


def build_selected_configs(args, vanilla_configs):
    requested = parse_csv(args.configs)
    photon_configs = {name: flags for name, flags in CONFIGS}
    selected = []

    explicit_vanilla = any(
        name in vanilla_configs and name != "baseline"
    ...
    for name in requested:
        ...
        if name == "baseline":
            if explicit_vanilla and not explicit_photon:
                selected.append(("baseline", vanilla_configs["baseline"]))
            else:
                selected += build_photon_configs(args, ["baseline"])
            continue

        if name in vanilla_configs:
            selected.append((name, vanilla_configs[name]))
            continue
...
def create_output_dir():
    ...
    output_dir = os.path.join(
        ROOT_DIR,
        "results",
        datetime.now().strftime("%Y-%m-%d-%H-%M-%S-vanilla-baseline"),
    )
