...
__kernel void llm_bert_embedding_synthetic(
    __global float *out,
    ...
    const __global float *token_type_table,
    int rows,
    int hidden,
    int vocab_size) {
  ...
  int token_type = token_type_ids[row] & 1;

  out[gid] = token_table[token * hidden + col] +
             position_table[row * hidden + col] +
             token_type_table[token_type * hidden + col];
}
