...
CONFIGS = [
    ("ptcl_mode", "MSHR coalescing", "#83CEE2", ""),
    ("flex_entry", "PTCL lookup table", "#31ADCE", ""),
    ("idle_iommu_assist", "Idle IOMMU assist", "#1D687C", ""),
    ("ptcl_mode_flex_iommu_assist", "PASTA", "#ED6612", ""),
]

BASELINE_PASTA_CONFIGS = [
    ("baseline", "Baseline", "#5ABED8", ""),
    ("ptcl_mode_flex_iommu_assist", "PASTA", "#ED6612", ""),
]

CONFIG_SUFFIXES = sorted(
    ["baseline", *(config for config, _, _, _ in CONFIGS)],
    key=len,
    reverse=True,
)
...
def build_speedups(
    ...
    speedups: dict[str, list[float]] = {config: [] for config, _, _, _ in configs}
    missing: list[str] = []
...
def annotate_clipped_bar(
    ax, bar, value: float, ylimit: float, x_nudge: float
) -> None:
...
def plot_speedups(
    ...
    ax.axvline(geomean_x - x_step / 2, color="#888888", linestyle=":", linewidth=0.7)
    ...
    handles_by_config = {
        config: Patch(facecolor=color, edgecolor="none", linewidth=0, label=legend)
