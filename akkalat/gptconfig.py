"""GPT decomposed llmop configuration.
...
def profile(profile_name):
    if profile_name not in PROFILES:
        raise ValueError(f"unknown GPT profile {profile_name!r}")
...
def gpt_ops(profile_config, split_k=1):
    ...
    for layer in range(layers):
        ...
        ops += [
            ...
            (f"{prefix}_causal_mask",
             op_flags(
                 "causal-mask", rows=rows, seq_len=seq_len,
                 batch_size=batch)),
